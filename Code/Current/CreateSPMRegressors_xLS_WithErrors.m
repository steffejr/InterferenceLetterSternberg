...
for i = 1:NTrials
                ...
                for k = 1:length(Phases)
                    if ~isempty(strfind(names{j},Phases{k}))
                        if k == 1
                            ...
                            %DUR(i,j) = DurTimes(k);
                            DUR(i,j) = 6+Trial.LetterResponseTime(max(find(Trial.LetterResponseTime>-99)));
    ...
    elseif strcmp(Trial.LetterResponseAcc,'FA') || strcmp(Trial.LetterResponseAcc,'MS')
        for j = 1:NCond
            %             % Find the names with the same Letter Load level
    ...
    elseif strcmp(Trial.LetterResponseAcc,'TO')
        for j = 1:NCond
            %             % Find the names with the same Letter Load level
            if ~isempty(findstr(names{j},'TO'))
